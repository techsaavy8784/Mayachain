...
func assertJSONStructTagsMatch(c *C, thor, spec interface{}) {
	mayaType := reflect.TypeOf(thor)
	specType := reflect.TypeOf(spec)
	comment := Commentf("mayaType=%s; specType=%s", mayaType.Name(), specType.Name())

	c.Assert(specType.NumField(), Equals, mayaType.NumField(), comment)
	for i := 0; i < mayaType.NumField(); i++ {
		specTag := specType.Field(i).Tag.Get("json")
		mayaTag := mayaType.Field(i).Tag.Get("json")
		c.Assert(specTag, Equals, mayaTag, comment)
	}
}

...
`MaximumLiquidityCacao`: Max cacao capped on the pools
...
`MinRunePoolDepth`: Minimum number of cacao to be considered to become active
`PoolCycle`: Number of blocks the network will churn the pools (add/remove new available pools)
`StagedPoolCost`: Number of cacao (1e10 notation) that a stage pool is deducted on each pool cycle.
...
`NativeTransactionFee`: The cacao fee for a native transaction (gas cost in 1e10 notation)

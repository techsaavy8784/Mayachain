...
type KujiBlockScanner struct {
	...
	// difference in base chain decimals (thorchain:1e8, kuji:1e6).
...
func (c *KujiBlockScanner) averageFee() ctypes.Uint {
	...
	c.logger.Debug().Str("fees", fmt.Sprintf("%v", c.feeCache)).Msg("averageFee")
	// compute mean
	...
	c.logger.Debug().Uint64("mean", mean.Uint64()).Msg("averageFee")
	return mean
}
	...
	for i, rawTx := range rawTxs {
		...
		if err != nil {
			if strings.Contains(err.Error(), "unable to resolve type URL") {
				...
				if strings.Contains(err.Error(), "MsgSend") { // || strings.Contains(err.Error(), "MsgExecuteContract") { // TODO: When we support MsgExecuteContract, remove this check

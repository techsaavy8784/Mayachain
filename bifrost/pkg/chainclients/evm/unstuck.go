...
// This is the number of MAYAChain blocks to wait before re-broadcasting a stuck tx with
...
func (c *EVMClient) unstuckAction() {
	...
	for _, item := range signedTxItems {
		...
		if item.Height > height {
			c.logger.Warn().Msg("signed outbound height greater than current mayachain height")
			continue
		}

...
func (s *BlockScannerTestSuite) SetUpSuite(c *C) {
	...
	s.bridge, err = mayaclient.NewMayachainBridge(s.cfg, s.m, s.keys)
	c.Assert(err, IsNil)
}
...
const (
	...
	lastBlockResult = `[ { "chain": "BNB", "last_observed_in": 1, "last_signed_out": 1, "mayachain": 3 }]`
)

func (s *BlockScannerTestSuite) TestBlockScanner(c *C) {
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch {
		case strings.HasPrefix(r.RequestURI, mayaclient.MimirEndpoint):
...
func (s *BlockScannerTestSuite) TestIsChainPaused(c *C) {
	...
	h := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		c.Logf("================>:%s", r.RequestURI)
		switch {
		case strings.HasPrefix(r.RequestURI, mayaclient.LastBlockEndpoint):

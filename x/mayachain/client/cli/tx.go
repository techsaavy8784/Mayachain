	...
	return func(cmd *cobra.Command, args []string) error {
		...
		if txids != "" {
			...
			for _, txid := range txidList {
				observation, err := findLackingObservation(txid, nodeAddress, mayaNodeAPI)
				if err != nil {
...
func findLackingObservation(txid, address, mayanodeAPI string) (*types.ObservedTx, error) {
	// get tx details from mayanode API
	url := fmt.Sprintf("%s/mayachain/tx/details/%s", mayanodeAPI, txid)

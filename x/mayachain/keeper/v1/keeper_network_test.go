...
func (KeeperNetworkSuite) TestPOL(c *C) {
	...
	c.Check(err, IsNil)
	c.Check(pol.CacaoDeposited.Equal(cosmos.ZeroUint()), Equals, true)

	pol.CacaoDeposited = cosmos.NewUint(common.One * 100)
	err1 := k.SetPOL(ctx, pol)
	c.Assert(err1, IsNil)

	pol2, err2 := k.GetPOL(ctx)
	c.Check(err2, IsNil)
	c.Check(pol2.CacaoDeposited.Uint64(), Equals, uint64(100*common.One))
}

...
func (s *MsgTssPoolSuite) TestMsgTssPool(c *C) {
	...
	msg, err := NewMsgTssPool(pks, pk, KeygenType_AsgardKeygen, 1, Blame{}, []string{common.BaseAsset().Chain.String()}, addr, keygenTime)
	c.Assert(err, IsNil)
	...
	msg, err = NewMsgTssPool(pks, pk, KeygenType_AsgardKeygen, 1, Blame{}, []string{common.BaseAsset().Chain.String(), common.BaseAsset().Chain.String()}, addr, keygenTime)
	c.Assert(err, IsNil)
	c.Check(msg.ValidateBasic(), NotNil)

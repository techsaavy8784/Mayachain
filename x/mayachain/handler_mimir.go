...
var (
	...
	mimirValidKeyV109 = regexp.MustCompile(constants.MimirKeyRegexV109).MatchString
)
...
func (h MimirHandler) validateV109(ctx cosmos.Context, msg MsgMimir) error {
	if err := msg.ValidateBasic(); err != nil {
		return err
	}
	if !mimirValidKeyV109(msg.Key) || len(msg.Key) > 64 {
	...
	if !h.isAdmin(msg.Signer) {
		...
		// Check if mimir key starts with SymbolicNodeMimirValue
		if msg.Key[:len(constants.SymbolicNodeMimirValue)] != constants.SymbolicNodeMimirValue {
			return errInvalidSymbolicNodeMimirValue
		}
	}
	return nil
}

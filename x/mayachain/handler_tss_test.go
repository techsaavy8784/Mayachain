...
func newTssHandlerTestHelper(c *C) tssHandlerTestHelper {
	...
	msg, err := NewMsgTssPool(members.Strings(), poolPk, AsgardKeygen, ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), signer, keygenTime)
	c.Assert(err, IsNil)
	voter := NewTssVoter(msg.ID, members.Strings(), poolPk)
	keeperHelper.SetTssVoter(ctx, voter)

	asgardVault := NewVault(ctx.BlockHeight(), ActiveVault, AsgardVault, GetRandomPubKey(), common.Chains{common.BaseAsset().Chain}.Strings(), []ChainContract{})
...
func (s *HandlerTssSuite) TestTssHandler(c *C) {
	...
	testCases := []struct {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				msg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), GetRandomValidatorNode(NodeActive).NodeAddress, keygenTime)
				c.Assert(err, IsNil)
				return msg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				msg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), cosmos.AccAddress{}, keygenTime)
				c.Assert(err, IsNil)
				return msg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				tssMsg.ID = ""
				return tssMsg
			},
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(nil, GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(common.PubKeys{GetRandomPubKey()}.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool([]string{GetRandomPubKey().String(), GetRandomPubKey().String(), ""}, GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), common.EmptyPubKey, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), "whatever", AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), helper.poolPk, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				...
				tssMsg, err := NewMsgTssPool(helper.members.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), b, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				voter, err := helper.keeper.GetTssVoter(helper.ctx, tssMsg.ID)
				c.Assert(err, IsNil)
				for _, pk := range helper.members {
		...
		},
		{
			...
			messageCreator: func(helper tssHandlerTestHelper) cosmos.Msg {
				...
				tssMsg, err := NewMsgTssPool(members.Strings(), GetRandomPubKey(), AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), helper.signer, keygenTime)
				c.Assert(err, IsNil)
				return tssMsg
			},
			runner: func(handler TssHandler, msg cosmos.Msg, helper tssHandlerTestHelper) (*cosmos.Result, error) {
	...
	for _, tc := range testCases {
		...
		mgr.slasher = newSlasherV92(mgr.K, mgr.eventMgr)
...
func (s *HandlerTssSuite) TestKeygenSuccessHandler(c *C) {
	...
	for idx, item := range helper.members {
		...
		c.Assert(err, IsNil)
		tssMsg, err := NewMsgTssPool(helper.members.Strings(), poolPubKey, AsgardKeygen, helper.ctx.BlockHeight(), Blame{}, common.Chains{common.BaseAsset().Chain}.Strings(), thorAddr, keygenTime)
		c.Assert(err, IsNil)

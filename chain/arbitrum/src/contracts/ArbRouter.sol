...
contract ArbRouter {
   ...
   * @param asset address - ERC20 asset or zero address for ETH
  ...
  /**
   * @notice Deposit an asset with a memo. ETH is forwarded, ERC-20 stays in ROUTER
   * @param vault address - vault address for router
   * @param asset address - ERC20 asset or zero address for ETH
   * @param amount uint - amount to deposit
   ...
   * @dev Note: Contract recipients of ETH are only given 2300 Gas to complete execution.
   ...
   * @dev Example Memo: "~1b3:ARB.0xFinalToken:0xTo:
   ...
   * @param finalToken address - ERC20 asset or zero address for ETH
  ...
  function transferOutAndCall(
    ...
    (bool erc20Success, ) = target.call{value: _safeAmount}(
  ...
  /**
   * @notice  A vault can call to "return" all assets to an asgard, including ETH.
   * @param router address - current vault address for router
   * @param asgard address - current address for asgard
   * @param coins Coin[] - ERC20/ETH in vault - { asset: address, amount: uint }
   * @param memo string - transaction memo
   */
  ...
  function safeTransferFrom(address _asset, uint256 _amount)
    ...
    returns (uint256 amount)
  {
    uint256 _startBal = IERC20(_asset).balanceOf(address(this));
   ...
   * @param _asset address - ERC20 asset or zero address for ETH
   * @param _amount uint - amount to transfer
   */
  function _routerDeposit(
    ...
    (bool success, ) = _asset.call(
      abi.encodeWithSignature("approve(address,uint256)", _router, _amount)
    ); // Approve to transfer

...
class TestAccount(unittest.TestCase):
    def test_addsub(self):
        ...
        acct.add([Coin("BNB.BNB", 20), Coin(CACAO, 100)])
        self.assertEqual(acct.get("BNB.BNB"), 45)
        self.assertEqual(acct.get(CACAO), 100)

        acct.sub([Coin("BNB.BNB", 20), Coin(CACAO, 100)])
        self.assertEqual(acct.get("BNB.BNB"), 25)
        self.assertEqual(acct.get(CACAO), 0)

...
from mayachain.mayachain import MayachainClient
from mayachain.midgard import MidgardClient
from utils.common import Coin, get_cacao_asset, get_diff
...
class Health:
    def __init__(
        ...
        self.mayachain_client = MayachainClient(maya)
        self.mayachain_pools = []
        self.mayachain_asgard_vaults = []
    ...
    def run(self):
        ...
        - check pools state between midgard and mayachain
    ...
    def retrieve_vaults(self):
        """Retrieve vault data from MAYAChain APIs."""
        self.mayachain_asgards = self.mayachain_client.get_asgard_vaults()
        for vault in self.mayachain_asgards:
            if vault["coins"]:
                vault["coins"] = [Coin.from_data(c) for c in vault["coins"]]

        self.mayachain_yggdrasils = self.mayachain_client.get_yggdrasil_vaults()
        for vault in self.mayachain_yggdrasils:
            if vault["coins"]:
                vault["coins"] = [Coin.from_data(c) for c in vault["coins"]]
    ...
    def check_pools(self):
        """Check pools state between Midgard and Mayachain APIs."""
        self.mayachain_pools = self.mayachain_client.get_pools()
        for tpool in self.mayachain_pools:
            asset = tpool["asset"]
            mpool = self.midgard_client.get_pool(asset)

            # Mayachain Coins
            tcacao = Coin(CACAO, tpool["balance_cacao"])
            tasset = Coin(asset, tpool["balance_asset"])
            tsynth = Coin(asset, tpool["synth_supply"])

            # Midgard Coins
            mcacao = Coin(CACAO, mpool["cacaoDepth"])
            masset = Coin(asset, mpool["assetDepth"])
            msynth = Coin(asset, mpool["synthSupply"])

            # Check balances
            diff = get_diff(tcacao.amount, mcacao.amount)
            if diff > self.margin_err:
                sub = abs(tcacao - mcacao)
                self.error(
                    f"Midgard   [{asset:15}] CACAO "
                    f" [T]{tcacao.str_amt()} != [M]{mcacao.str_amt()}"
            ...
            tpool_price = int(tpool["balance_cacao"]) / int(tpool["balance_asset"])
